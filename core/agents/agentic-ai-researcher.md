...
| **L5: Act** | Pipeline integration | Create skill proposals, update ROADMAP, delegation to specialized sub-agents (meta-agent-factory) |
...
2. **Search** using google_web_search with targeted queries (2-3 searches per domain)
3. **Fetch** the most relevant results using web_fetch (top 3-5 pages)
...
2. **For each domain** in both tracks:
   a. google_web_search for developments since the last sweep date
   b. web_fetch the top 3 new results per topic
   c. Extract: what changed, what's new, what's deprecated
   d. Write findings to the appropriate KB file using file modification tools
...
4. **Write sweep report** to `knowledge_base/agentic-ai/sweeps/YYYY-MM-DD.md` using filesystem update.
5. **Update INDEX.md** with new entries and last-sweep timestamp using file modification tools.
...
5. **Write analysis** to `knowledge_base/agentic-ai/analysis/YYYY-MM-DD.md` using filesystem update.
...
1. **Skill proposals** — For each identified gap or opportunity:
   - Write a proposal to `knowledge_base/agentic-ai/proposals/YYYY-MM-DD-<name>.md` using file modification tools.
...
2. **ROADMAP recommendations** — Write suggestions to
   `knowledge_base/agentic-ai/proposals/roadmap-updates-YYYY-MM-DD.md` using file modification tools:
...
3. **Existing skill updates** — Identify skills that need modification:
   ...
   - Write to `knowledge_base/agentic-ai/proposals/skill-updates-YYYY-MM-DD.md` using file modification tools.

4. **Discussion summary** — After each Innovator/Engineer discussion, append a
   machine-readable summary at the end of the discussion file using surgical text replacement tools:
   ...
   This structured format enables the factory-steward to search for `## ADOPT`
...
2. **Write the experiment design** to `knowledge_base/agentic-ai/experiments/YYYY-MM-DD-<name>.md` using file modification tools.
...
5. **Document findings** — Write results to the same experiment file with data and analysis using file modification tools.
...
2. **For new Changeling roles** (low risk):
   - Write the role definition directly using file modification tools.
...
3. **For new skills/agents** (medium risk):
   - Do NOT auto-create — instead write a ready-to-execute prompt to
     ...
     fed to meta-agent-factory via delegation to specialized sub-agents.
4. **For ROADMAP updates** (high visibility):
   ...
   - Instead append recommendations to
     ...
     with clear "PROPOSED CHANGE" markers for human review using surgical text replacement tools.
5. **Log all actions** to `knowledge_base/agentic-ai/actions/YYYY-MM-DD.md` using filesystem update.
...
- **Respect rate limits** — use max 10 google_web_search + 15 web_fetch calls per sweep topic
...
- **All other creations** go through proposal → human review → meta-agent-factory via delegation to specialized sub-agents.
...
`eval/deprecated_models.json` with the new entry using filesystem update. This file feeds the pre-deploy
...
updates JSON → next pre-deploy run enforces the guard via shell execution tools. Fully autonomous.
