...
- Apply changes to the target `SKILL.md` using file modification tools.
- Run evaluation; if pass rate improves, commit via shell execution tools; otherwise, revert.
...
- Iteratively rewrite Skill instructions so that lightweight models (Haiku) can
  reach >= 90% of the flagship model's performance baseline using surgical text replacement tools.

## State Persistence & Crash Recovery

To survive interruptions (quota exhaustion, context limits, timeouts), persist
optimization state to `eval/experiment_log.json` after every iteration using filesystem updates.
...
After evaluating each proposal, update `eval/experiment_log.json` with file modification tools:
...
2. **Baseline**: If not resuming, measure the current version using shell execution tools to run `python3 eval/run_eval_async.py <skill-path> --no-cache`.
...
3. **Loop (until target rate or max iterations reached)**:
   ...
   - **Evaluate**: For each proposal:
     - Apply change to `SKILL.md` using surgical text replacement tools.
     - Run `bash eval/check-permissions.sh <skill-path>` via shell execution tools. If fail, REVERT immediately.
     - Run `python3 eval/run_eval_async.py <skill-path> --no-cache` via shell execution tools.
     - Record T-set posterior mean and CI.
   - **Persist**: Update `eval/experiment_log.json` with iteration results, `best_so_far`, and `current_description` using file modification tools.
   - **Decide**: Compare the best proposal's T-set results against the current baseline using shell execution tools to run `python3 eval/bayesian_eval.py --compare old.json new.json`.
     - **Commit rule**: Execute `git commit` via shell execution tools ONLY if the new CI lower bound is strictly greater than the old CI upper bound (no overlap).
     - **Update baseline**: If committed, the new version becomes the baseline for the next iteration.
     - **Revert**: If no proposal meets the non-overlap rule, use shell execution tools to run `git checkout -- <skill-path>` and try a different strategy.
