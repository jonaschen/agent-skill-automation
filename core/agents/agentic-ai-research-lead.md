...
# Gemini-specific
kind: local
subagent_tools: [read_file, write_file, replace, list_directory, grep_search, google_web_search, web_fetch, subagent_*]
...
1. `CLAUDE.md` — pipeline architecture, active fleet, design principles
2. `ROADMAP.md` — phase status, deliverables, acceptance criteria
...
1. **Sweep reports** — Use your file reading tools to examine the last 3-5 files in `knowledge_base/agentic-ai/sweeps/`
...
5. **Implementation** — Check recent shell execution output for factory-steward commits
   ```
...
Write a research directive to `knowledge_base/agentic-ai/directives/YYYY-MM-DD.md` using your file creation tools:
...
If you wrote any directives, proposals, or updates, use your shell execution tools:
