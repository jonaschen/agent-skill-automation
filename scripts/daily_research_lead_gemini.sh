#!/bin/bash
# daily_research_lead_gemini.sh — Cron-triggered research direction session (Gemini version)
#
...
#
# Usage: Called by cron, or manually: ./scripts/daily_research_lead_gemini.sh
...
recover_uncommitted() {
  ...
  if [ -n "$dirty" ]; then
    ...
    git commit -m "research-lead-gemini(auto): $session_name uncommitted work ($DATE)" >> "$log_file" 2>&1 || true
  fi
}

START_TIME=$(date +%s)
init_session_log "research-lead-gemini" "$REPO_ROOT"
...
{
  "agent": "agentic-ai-research-lead-gemini",
  "date": "$DATE",
...
PERF_EOF
  ...
  check_cost_ceiling "research-lead-gemini" "$duration" "$PERF_DIR" "$SECURITY_LOG_DIR" 2>> "$LOG_FILE" || true
  ...
  find "$LOG_DIR" -name "research-lead-gemini-*.log" -mtime +30 -delete 2>/dev/null
  find "$PERF_DIR" -name "research-lead-gemini-*.json" -mtime +30 -delete 2>/dev/null
}

# Start incremental watchdog
WATCHDOG_PID=$(start_incremental_watchdog "research-lead-gemini" "$$" "$PERF_DIR" "$SECURITY_LOG_DIR")
...
4. Phase 3 — Set Priorities & Formulate Hypotheses: Write a research directive to knowledge_base/agentic-ai/directives/${DATE}.md. The directive MUST include a 'Strategic Alignment' section showing how P0/P1 topics relate to the owner's strategic priorities. In addition, propose a formal, testable Academic Hypothesis (e.g. \"Does method X improve metric Y?\") based on recent findings, and write it to knowledge_base/agentic-ai/hypotheses/${DATE}-hypothesis.md.
...
6. Phase 5 — Commit: git add knowledge_base/agentic-ai/directives/ knowledge_base/agentic-ai/proposals/ knowledge_base/agentic-ai/hypotheses/ && git commit
