#!/bin/bash
# daily_android_sw_steward_gemini.sh — Cron-triggered daily Android-Software stewardship session (Gemini version)
#
# Runs the android-sw-steward-gemini agent to advance Phase 4 deliverables,
# perform gap analysis, and research AOSP updates.
#
# Usage: Called by cron, or manually: ./scripts/daily_android_sw_steward_gemini.sh
...
START_TIME=$(date +%s)
init_session_log "android-sw-gemini" "$REPO_ROOT"
...
{
  "agent": "android-sw-steward-gemini",
  "date": "$DATE",
...
PERF_EOF
  ...
  check_cost_ceiling "android-sw-gemini" "$duration" "$PERF_DIR" "$SECURITY_LOG_DIR" 2>> "$LOG_FILE" || true
  ...
  find "$LOG_DIR" -name "android-sw-gemini-*.log" -mtime +30 -delete 2>/dev/null
  find "$PERF_DIR" -name "android-sw-gemini-*.json" -mtime +30 -delete 2>/dev/null
}

# Start incremental watchdog
WATCHDOG_PID=$(start_incremental_watchdog "android-sw-gemini" "$$" "$PERF_DIR" "$SECURITY_LOG_DIR")
...
(cd "$TARGET_REPO" && timeout 2400 "$GEMINI" --approval-mode yolo -p "You are the android-sw-steward agent (Gemini). Read $REPO_ROOT/.gemini/agents/android-sw-steward.md for your full instructions.

Execute a stewardship session:
1. Orient: Read all mandatory documents (CLAUDE.md, ANDROID_SW_OWNER_DEV_PLAN.md, ROADMAP.md, README.md, .gemini/steering-notes.md if it exists)
