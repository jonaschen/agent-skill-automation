#!/bin/bash
# daily_ltc_steward_gemini.sh — Cron-triggered daily long-term-care-expert stewardship session (Gemini version)
#
# Runs the ltc-steward-gemini agent to advance ROADMAP phases, run evaluations,
# maintain compliance, and research elderly care topics.
#
# Usage: Called by cron, or manually: ./scripts/daily_ltc_steward_gemini.sh
...
DATE=$(date +"%Y-%m-%d")
LOG_FILE="$LOG_DIR/ltc-gemini-${DATE}.log"
PERF_FILE="$PERF_DIR/ltc-gemini-${DATE}.json"
...
START_TIME=$(date +%s)
init_session_log "ltc-gemini" "$REPO_ROOT"
...
PERF_EOF
  ...
  check_cost_ceiling "ltc-gemini" "$duration" "$PERF_DIR" "$SECURITY_LOG_DIR" 2>> "$LOG_FILE" || true
...
1. Orient: Read ALL mandatory documents (CLAUDE.md, ROADMAP.md, LONGTERM_CARE_EXPERT_DEV_PLAN.md, DIGITAL_SURROGATE_AGILE_POC.md, HANA_CHARACTER_SPEC.md, SAYO_PREFERENCE_SYSTEM.md, .gemini/steering-notes.md if it exists)
2. **BLOCKING — Read steering notes**: Read $TARGET_REPO/.gemini/steering-notes.md (if it exists). Address ALL P0 items BEFORE any other work. Also check $REPO_ROOT/knowledge_base/steward-reviews/ for the latest review file.
...
At the end, output a brief JSON summary: {\"deliverable\": \"...\", \"status\": \"...\", \"files_changed\": [...], \"tests_passed\": true/false, \"compliance_violations\": 0}" < /dev/null) >> "$LOG_FILE" 2>&1 || true
...
if [ "$PHASE_WORK_DURATION" -lt 60 ]; then
  ...
  echo "[SKIP] Skipping research session to avoid burning quota. Human action required: check GEMINI_API_KEY in $TARGET_REPO/.env" >> "$LOG_FILE"
...
5. Commit: If you made any changes, stage and commit with message 'research-gemini: <topic> ($DATE)'
