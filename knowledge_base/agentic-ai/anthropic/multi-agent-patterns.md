...
### 2026-04-23 — Session Rewind and "Chapters" for Long-Horizon Persistence
- **What**: Released alongside Opus 4.7, **Session Rewind** allows agents to roll back their filesystem and conversation state to any previous `message.uuid` checkpoint. Complementing this is **Chapters**, a semantic summarization pattern that compresses completed sub-tasks into "chapter summaries" within the context window.
- **Significance**: This directly addresses "context wall" issues in multi-hour autonomous sessions. Session Rewind enables our Phase 4 optimizer to implement a "backtracking" search — if a skill optimization path fails evaluation, the agent can rewind and try a different branch without re-running the entire session.
- **Source**: [anthropic.com/engineering/agent-persistence](https://www.anthropic.com/engineering/agent-persistence)
