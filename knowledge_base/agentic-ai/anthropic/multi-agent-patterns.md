...
- **Source**: https://code.claude.com/docs/en/agent-teams, https://www.anthropic.com/engineering/multi-agent-research-system (verified Apr 21)
...
- **Significance**: **(1) Agent teams crash fix** — if our Phase 5 topology uses agent teams, v2.1.114 is required. **(2) Routines limitations confirm our cron infrastructure superiority** — 1h minimum interval, limited to `claude/` branches, no custom performance logging, no multi-step pipeline chaining. Our cron runs every hour with exact timing, push to any branch, write JSON performance records, and chain 3 agents per cycle. Routines may work for simpler single-agent tasks (e.g., replacing the LTC steward's cron entry) but cannot replace our multi-agent pipeline. **(3) Five-pattern taxonomy** — the Message Bus and Shared State patterns are new additions vs. the four-pattern docs. These should inform Phase 5 TCI routing when it activates.
- **Source**: https://github.com/anthropics/claude-code/releases, https://code.claude.com/docs/en/routines, https://resources.anthropic.com/2026-agentic-coding-trends-report
