...
### 2026-04-28 (afternoon) — v2.1.119/v2.1.120 Community Regressions: 8 Filed Apr 24 + 3 More Apr 27

- **What**: A community-curated gist (yurukusa, published Apr 24) catalogs **eight regressions on v2.1.119/v2.1.120** with workarounds and a global rollback recommendation to **v2.1.117**:
  - **#53044, #53041** — `claude --resume` crashes with `TypeError` at startup (v2.1.120-only). Workaround: roll back to v2.1.117.
  - **#53031** — Silent routing of `claude-opus-4-7` (200k) to the 1M-context variant without notice. Workaround: verify with `/status` after `/model`.
  - **#53038** — Terminal resize causes UI duplication; regression of prior fix #49086. Workaround: `Ctrl+L` after resize.
  - **#53028** — Auto-update mechanism stopped functioning silently (v2.1.120-only). Workaround: manual `claude update`.
  - **#53035** — `/mcp` menu freezes when using `--resume` in WSL2.
  - **#53040** — `CLAUDE.md` loaded but model ignores it despite low context usage.
  - **#53012** — `sandbox.excludedCommands` doesn't bypass network enforcement on macOS.
  - **#53015** — Worktree creation hangs on macOS 26.4; git merge blocks indefinitely.

  **Three additional bugs filed Apr 27** that extend or complement the gist:
  - **#53976** — `/login` and `/mcp` auth modals freeze inside `--resume` sessions on v2.1.119 (macOS, Ghostty). Same family as #53035. Force-quit is the only escape.
  - **#53972** — `claude` subprocess does not inherit the active OTel span as parent. Requires manual `OTEL_TRACEPARENT` injection across `subprocess.Popen` boundaries. Direct relevance to any future OTEL instrumentation of cron pipelines.
  - **#53973** — Claude 4.7 ignores established project constraints and over-generates code/docs despite low context usage. Possibly a sibling complaint to #49562's resolved harness regressions; single data point.
- **Significance**: HIGH (operational). The gist materially revises the Apr 28 morning recommendation. Morning suggested upgrading to v2.1.119 (documented) over v2.1.120 (silent). Afternoon evidence shows v2.1.120 is **actively breaking** for `--resume` users and has a silently broken auto-update path. For our cron pipeline (which doesn't use `--resume`) the safer pin shifts to **v2.1.119** for the documented `PostToolUse.duration_ms` and parallel MCP-server reconfig wins; **v2.1.117** is safest if avoiding all v2.1.119/.120 regressions. **#53972 (OTel parent-span loss)** is the highest-impact item for future S1 work — if/when we wire OTEL across the researcher → research-lead → factory-steward chain, traces will be unparented unless we forward `traceparent`.
- **Source**: https://gist.github.com/yurukusa/a866b4cd2976486156a00c190c39cef6, https://github.com/anthropics/claude-code/issues/53972, https://github.com/anthropics/claude-code/issues/53973, https://github.com/anthropics/claude-code/issues/53976

### 2026-04-28 — v2.1.120 Silent npm Release (no GitHub Release, no CHANGELOG entry)
- **What**: CC v2.1.120 published to npm Apr 24 23:02 UTC (`npm view @anthropic-ai/claude-code time`) and a git tag `v2.1.120` exists, but no GitHub Release was cut and the version is **absent from `CHANGELOG.md`**. The version is referenced in Agent SDK Python v0.1.67 release notes ("Updated bundled Claude CLI to version 2.1.120"), confirming the build exists. v2.1.119 remains the "Latest" tag on GitHub Releases. Today (Apr 28) is day 4 of v2.1.120 with no public changelog.
- **Significance**: MEDIUM. Anthropic appears to have published a silent patch — the SDK bundles it, but no release notes describe what changed. Possibilities: (a) emergency fix being held back from announcement, (b) tooling/build hiccup that skipped CHANGELOG generation, (c) deliberate "shadow" release for SDK internal use only. For our shadow eval, this affects which CC version Jonas should target — v2.1.119 (documented) or v2.1.120 (latest npm tag, undocumented).
- **Source**: `npm view @anthropic-ai/claude-code time --json`, `gh api repos/anthropics/claude-code/tags`, https://github.com/anthropics/claude-agent-sdk-python/releases/tag/v0.1.67

### 2026-04-23 — v2.1.119 Released (Apr 23 23:24 UTC, 8h after v2.1.118): /config Persistence, --from-pr Multi-Provider, prUrlTemplate, PostToolUse duration_ms
- **What**: v2.1.119 is a same-day follow-up to v2.1.118 (8 hours later). Headlines: (1) **`/config` settings now persist** to `~/.claude/settings.json` and participate in the project/local/policy override precedence — theme, editor mode, verbose, etc. (2) **`prUrlTemplate` setting** points the footer PR badge at a custom code-review URL instead of github.com. (3) **`CLAUDE_CODE_HIDE_CWD`** env var hides the working directory in the startup logo. (4) **`--from-pr` now accepts GitLab MR, Bitbucket PR, and GitHub Enterprise PR URLs** (previously GitHub.com only). (5) **`--print` mode honors agent frontmatter** `tools:` / `disallowedTools:`, matching interactive mode. (6) **`--agent <name>` honors `permissionMode`** for built-in agents. (7) **PowerShell auto-approval** in permission mode (matches Bash). (8) **`PostToolUse` and `PostToolUseFailure` hook inputs include `duration_ms`** (tool execution time, excluding permission prompts and PreToolUse hooks). (9) **Subagent and SDK MCP server reconfiguration runs in parallel** (was serial). (10) Plugins pinned by another plugin's version constraint auto-update to highest satisfying git tag. (11) Vim mode: Esc in INSERT no longer pulls queued message back into input. (12) Slash command suggestions highlight matched chars; long descriptions wrap instead of truncate. (13) `owner/repo#N` shorthand uses the git remote's host. (14) `blockedMarketplaces` now correctly enforces `hostPattern`/`pathPattern`. (15) OTEL: `tool_result`/`tool_decision` events include `tool_use_id`; `tool_result` adds `tool_input_size_bytes`. (16) Status line stdin JSON now includes `effort.level` and `thinking.enabled`. (17) Tool search **disabled by default on Vertex AI** (unsupported beta header) — opt in with `ENABLE_TOOL_SEARCH`. (18) ~25 bug fixes including CRLF paste, kitty keyboard protocol, Glob/Grep disappearing when Bash denied, fullscreen scroll snap-back, MCP HTTP OAuth discovery non-JSON bodies, async PostToolUse empty payloads, async PostToolUse empty entries, `${ENV_VAR}` substitution in MCP HTTP/SSE/WebSocket headers, `/skills` Enter behavior, `/agents` detail view labeling, plugin MCP servers on Windows with incomplete plugin cache, `/usage` progress bar overlap.
- **Significance**: HIGH for our pipeline. (a) **`PostToolUse` `duration_ms` is a direct S1 observability gain** — we can now measure per-tool execution time without external timing. This eliminates a measurement gap for our optimizer eval logging. (b) **Parallel MCP server reconfiguration** reduces subagent spawn latency (relevant to S2 orchestration experiments). (c) **`--print` mode honoring frontmatter** closes a long-standing parity gap — our daily cron scripts use `claude -p` and previously could not enforce per-agent tool permissions; now they can. (d) **`prUrlTemplate`** lets us route PR badges to internal Phabricator/Gerrit if needed for the AaaS phase. (e) **Tool search Vertex AI default-off** is a config compatibility break for any users running our agents on Vertex — they must set `ENABLE_TOOL_SEARCH` explicitly.
- **Source**: https://github.com/anthropics/claude-code/releases/tag/v2.1.119, https://raw.githubusercontent.com/anthropics/claude-code/main/CHANGELOG.md

### 2026-04-23 — Anthropic Public Postmortem: Three Engineering Missteps Caused Claude Code Decline (#49562 root cause confirmed)
- **What**: Anthropic published a postmortem (covered by VentureBeat and Yahoo Finance Apr 25-27) attributing Claude Code's monthlong perceived degradation to three specific engineering changes: (1) **Mar 4** — default reasoning effort reduced from "high" to "medium" to decrease latency (reverted Apr 7 after user feedback). (2) **Mar 26** — caching bug supposed to clear stale thinking from idle sessions instead cleared every turn, making Claude appear forgetful and burning usage limits faster (fixed Apr 10). (3) **Apr 16** — system prompt instruction added to cap responses at ~25 words between tool calls, measurably hurting code quality (reverted Apr 20 in v2.1.116). All three resolved by Apr 20. Anthropic reset usage limits for affected subscribers Apr 23. Quote: "This isn't the experience users should expect from Claude Code." Anthropic acknowledged the regression after weeks of community pressure.
- **Significance**: HIGH. **#49562 root cause is now public.** The issue was opened Apr 16 — the same day the verbosity prompt was added. The Apr 19 community comment ("I would love to revert back to Opus 4.6") was likely reacting to the Apr 16 verbosity change, not Opus 4.7 itself. The Opus 4.7 model was not the regression source; the harness/system-prompt changes were. This validates: (a) our Bayesian eval discipline (raw pass-rate dips need CI-overlap test before attributing to model change), (b) shadow eval as a tool for catching harness-driven regressions independent of the model. **Our shadow eval design — eval with a fixed model across CC versions — is exactly the methodology that would have caught this earlier**. For #49562 specifically: the bug is fixed but the issue is still OPEN with no staff response (12 days). Per directive's day-10 P2 downgrade rule (Apr 28 = day 12), downgrade now.
...
### 2026-04-22 — v2.1.117 (FREEZE BROKEN — major release)
- **What**: Major release with 25+ changes. Headlines: (1) **Fixed Opus 4.7 context window computation** — CC was computing against 200K instead of native 1M, causing inflated `/context` percentages and premature autocompacting. (2) **Forked subagents on external builds** via `CLAUDE_CODE_FORK_SUBAGENT=1`. (3) **Agent frontmatter `mcpServers`** now loaded for main-thread agent sessions via `--agent`. (4) **Concurrent MCP server connections** default for faster startup. (5) **Native `bfs`/`ugrep`** replacing Glob/Grep tools on macOS/Linux native builds. (6) **Default effort `high`** for Pro/Max on Opus 4.6/Sonnet 4.6 (was `medium`). (7) **Advisor Tool** experimental improvements — stability fixes, "experimental" label, startup notification. (8) **OTEL enhancements** — `effort` attribute on cost/token/api events, `command_name`/`command_source` for slash commands. (9) **Managed-settings** `blockedMarketplaces` and `strictKnownMarketplaces` enforced. (10) `/resume` offers to summarize stale sessions. (11) `/model` selections persist across restarts. (12) Plugin dependency auto-resolution. (13) `cleanupPeriodDays` sweep covers tasks/, shell-snapshots/, backups/. (14) Multiple bug fixes: OAuth token refresh, WebFetch hang on large pages, proxy 204 crash, NO_PROXY not respected under Bun, idle re-render loop on Linux, subagent malware warning false positive, Bedrock inference profile 400 with Opus 4.7, MCP elicitation auto-cancel in SDK mode.
- **Significance**: The Opus 4.7 context window fix (200K→1M) is the most impactful change — it directly addresses premature autocompacting that was likely contributing to the token burn complaints in #49562. The `CLAUDE_CODE_FORK_SUBAGENT=1` env var enables forked subagents on external (non-Anthropic) builds, relevant to S2. Native `bfs`/`ugrep` is a significant infrastructure change for search performance. OTEL `effort` attribute enables external monitoring of adaptive thinking behavior — S1 observability gain.
