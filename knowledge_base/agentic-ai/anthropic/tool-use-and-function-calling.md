...
- **Significance**: **Task Budgets investigation is now closed** (directive P1 resolved). For our pipeline: **(1) Use `--max-budget-usd` for steward sessions** — this is the supported CLI mechanism for cost control. `--max-budget-usd 10.00 --max-turns 50` would bound steward sessions. **(2) Task Budgets via API** — only relevant if we migrate steward agents from `claude -p` to the Agent SDK's `query()` function. At that point, `task_budget` can be passed via `output_config`. Not actionable now. **(3) No code changes needed** — our pipeline continues using `--max-turns` for iteration limits. Consider adding `--max-budget-usd` as an additional guard.
- **Source**: https://platform.claude.com/docs/en/build-with-claude/task-budgets, https://code.claude.com/docs/en/cli-reference, https://backgroundclaude.com/cli-reference

### 2026-04-18 -- Tool Use Stabilization Day 2 Post-Opus 4.7: Task Budgets Beta Confirmed; Opus 4.7 Breaking Changes on Sampling Parameters
- **What**: No new tool use API changes since April 16. **Post-Opus 4.7 tool use status confirmed**: (1) **Task Budgets** (public beta, `task-budgets-2026-03-13` header) — Opus 4.7 only feature: gives Claude an advisory token budget across a full agentic loop (thinking + tool calls + results + output). Model sees a running countdown and uses it to prioritize work. `output_config.task_budget: {type: "tokens", total: N}` where minimum is 20K tokens. Not a hard cap; model self-moderates. Distinct from `max_tokens` (hard per-request ceiling the model is unaware of). (2) **Opus 4.7 breaking changes relevant to tool use**: (a) `temperature`, `top_p`, `top_k` set to non-default values → 400 error. Must omit entirely. (b) `thinking: {type: "enabled", budget_tokens: N}` → 400 error. Must use `thinking: {type: "adaptive"}` with `output_config.effort`. (c) Thinking content omitted from response by default — set `display: "summarized"` to restore. (d) New tokenizer uses 1.0–1.35× more tokens — update `max_tokens` parameters. (3) **Programmatic Tool Calling** confirmed fully GA on 5 models (Opus 4.7, Opus 4.6, Sonnet 4.6, Opus 4.5, Sonnet 4.5) via Claude API and Microsoft Foundry. MCP tools still cannot be called programmatically. (4) **Advisor Tool** remains in beta (`advisor-tool-2026-03-01`). (5) All other GA tools unchanged: Tool Search, Tool Use Examples, fine-grained streaming, code execution, web search/fetch, memory tool.
- **Significance**: Task Budgets is the most significant new tool use feature for agentic workflows. For our pipeline: (1) **Task Budgets for steward agents** — we could set a 500K-token budget per steward session to prevent runaway token consumption during autonomous runs. Currently our steward sessions have no built-in token ceiling. **Action: evaluate adding task_budget to steward agent SDK configurations.** (2) **Sampling parameter removal** (temperature, top_p, top_k) is a breaking change that must be addressed before upgrading any pipeline component to Opus 4.7 — grep our codebase for these parameters and remove them. (3) **Tokenizer change (1.0–1.35× more tokens)** means our eval suite token budgets and compaction triggers need headroom increases. The 35% worst case could push some of our eval prompts over budget.
- **Source**: https://platform.claude.com/docs/en/about-claude/models/whats-new-claude-4-7, https://platform.claude.com/docs/en/build-with-claude/task-budgets, https://platform.claude.com/docs/en/release-notes/overview
