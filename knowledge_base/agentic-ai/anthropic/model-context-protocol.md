...
- **Significance**: Steady state. No change from evening.
- **Source**: https://www.rockcybermusings.com/p/weekly-musings-top-10-ai-security-20260410-20260416, https://www.ox.security/blog/mcp-supply-chain-advisory-rce-vulnerabilities-across-the-ai-ecosystem

### 2026-04-20 (evening) — MCP STDIO: No New CVEs; Stance Unchanged; Steady State
- **What**: No new MCP CVEs, spec changes, or Anthropic stance changes since morning sweep. STDIO vulnerability landscape remains at 10+ CVEs, 150M+ downloads affected, 4 exploitation families. Anthropic continues to maintain STDIO execution model is "by design." No new protocol patches or security advisories. CheckPoint published CVE-2025-59536 / CVE-2026-21852 research on Claude Code project file hooks (RCE + API token exfiltration) — these are Claude Code hook vulnerabilities, not MCP protocol-level issues.
- **Significance**: Steady state. The CheckPoint CVEs (hooks-based) are distinct from the STDIO transport class. Phase 5 argument-pattern validation remains the priority mitigation for our pipeline.
- **Source**: https://research.checkpoint.com/2026/rce-and-api-token-exfiltration-through-claude-code-project-files-cve-2025-59536/, https://www.theregister.com/2026/04/16/anthropic_mcp_design_flaw/

### 2026-04-20 — MCP STDIO Vulnerability Escalation: 10+ CVEs Across AI Ecosystem; Anthropic Maintains "Expected Behavior" Stance; OX Security "Mother of All AI Supply Chains" Report
- **What**: **MCP STDIO vulnerability landscape expanded significantly** since CVE-2026-40933. OX Security published a comprehensive report ("The Mother of All AI Supply Chains") identifying systemic command injection via MCP STDIO affecting **150M+ downloads, 7,000+ public servers, ~200,000 vulnerable instances**. **New CVEs since last entry**: CVE-2026-30623 (LiteLLM), CVE-2026-30624 (Agent Zero), CVE-2026-30618 (Fay Framework), CVE-2026-33224 (Bisheng/Jaaz), CVE-2026-30617 (Langchain-Chatchat), CVE-2026-30625 (Usopnic), CVE-2026-30615 (Windsurf), CVE-2026-26015 (DocsGPT), CVE-2025-65720 (GPT Researcher), GHSA-c9gw-hvqq (Flowise — the original CVE-2026-40933). Total: **10+ CVEs, 9 critical-rated**. **Four exploitation families identified**: (1) Direct UI injection of malicious MCP configs, (2) Hardening bypass via `npx` flags circumventing allowlists, (3) Prompt injection modifying MCP config files, (4) Hidden STDIO config MITM transport substitution. **Anthropic's response** (per The Register, Apr 19): rejected protocol patch, stating "This is an explicit part of how MCP stdio servers work and we believe this design does not represent a secure default." Quietly released updated security policy advising STDIO adapters "should be used with caution." **No MCP spec changes announced.**
- **Significance**: This is now a systemic class vulnerability, not an isolated CVE. The core issue — STDIO transport executing arbitrary OS commands — is architectural and won't be fixed at the protocol level given Anthropic's stance. **Impact on our pipeline**: (1) Our `cmd_chain_monitor.sh` uses binary-level allowlisting, which is exactly the pattern the `npx` bypass defeats (hardening bypass family #2). Phase 5 blocking mode MUST add argument-pattern validation. (2) All MCP STDIO usage in our pipeline should be audited against the four exploitation families. (3) Production deployments should use Streamable HTTP transport exclusively. (4) The "expected behavior" stance means downstream frameworks (Flowise, LangChain, etc.) bear full remediation responsibility. Our pipeline's MCP usage is local-only, mitigating network exposure, but the argument-blind allowlist gap remains.
- **Source**: https://www.theregister.com/2026/04/19/ai_vendors_response_to_security, https://www.cyberkendra.com/2026/04/anthropics-mcp-design-flaw-enables.html, https://www.computing.co.uk/news/2026/security/flaw-in-anthropic-s-mcp-putting-200k-servers-at-risk, https://www.infosecurity-magazine.com/news/systemic-flaw-mcp-expose-150/

### 2026-04-19 (evening-2) — CVE-2026-40933: Flowise MCP STDIO RCE (Critical, Active Exploitation)
- **What**: **NEW CVE** — CVE-2026-40933 (disclosed April 16, 2026) — authenticated RCE via MCP STDIO adapters in Flowise AI. Unsafe serialization of stdio commands in the MCP adapter bypasses input sanitization (validateCommandInjection, validateArgsForLocalFileAccess) because allowed commands like `npx` can be combined with code execution arguments (`-c touch /tmp/pwn`). CVSS critical (AV:N/AC:L/PR:L/UI:N/S:C/C:H/I:H/A:H). **12,000+ Flowise instances actively exposed** per The Hacker News. Active exploitation confirmed. Fixed in Flowise 3.1.0. Total MCP STDIO CVE count now **11+**.
- **Significance**: First confirmed active exploitation of MCP STDIO vulnerabilities in production AI infrastructure. The bypass pattern (using allowed binary names with code-execution arguments) is a class of vulnerability that affects any command allowlist checking only binary names. Relevant to our `cmd_chain_monitor.sh` which uses binary-level allowlisting. Phase 5 blocking mode should include argument-pattern validation. Reinforces STDIO-only-for-dev, Streamable HTTP for production policy.
- **Source**: https://github.com/advisories/GHSA-c9gw-hvqq-f33r, https://thehackernews.com/2026/04/flowise-ai-agent-builder-under-active.html, https://gbhackers.com/critical-flowise-flaw-enables-remote-command-execution/

### 2026-04-19 (evening) — MCP STDIO Vulnerability: No New CVEs; Anthropic Stance Unchanged; MCP Inspector CVE-2025-49596 Noted (Historical)
- **What**: **MCP STDIO vulnerability status check** (directive P1): No new CVEs assigned since last check. Anthropic maintains "expected behavior" / "secure default" stance. CSO Online confirms "10 have received CVE IDs" with "30+ responsible disclosures" total. Multiple frameworks (LangChain, FastMCP) declined fixes, citing mitigation through other controls. Recommendation from OX Security: implement command allowlist blocking sh, bash, powershell, curl, rm. **Historical note**: CVE-2025-49596 (CVSS 9.4, Critical) — MCP Inspector RCE via missing authentication between Inspector client and proxy server — was **patched in June 2025** (MCP Inspector 0.14.1+). Not new but not previously tracked in our KB.
- **Significance**: MCP STDIO vulnerability is in steady state — no resolution expected given Anthropic's stance. For our pipeline: STDIO remains the default for local MCP servers, but our Phase 5/7 deployments should prefer Streamable HTTP. The MCP Inspector CVE is historical/resolved — no action needed, but validates the importance of keeping MCP tooling up to date.
- **Source**: https://www.csoonline.com/article/4159889/rce-by-design-mcp-architectural-choice-haunts-ai-agent-ecosystem.html, https://www.oligo.security/blog/critical-rce-vulnerability-in-anthropic-mcp-inspector-cve-2025-49596

### 2026-04-19 (afternoon) — MCP STDIO Vulnerability: Expanded Coverage Confirms 150M Downloads at Risk; Industry-Wide Security Databases Emerging
- **What**: The MCP STDIO vulnerability (Ox Security, first reported Apr 15-16) is receiving broadening industry coverage and the impact scope has been upgraded. **Infosecurity Magazine** (Apr 15): "Systemic Flaw in MCP Protocol Could Expose 150 Million Downloads" — 200+ open source projects, 7,000+ publicly accessible servers, up to 200,000 vulnerable instances, **150 million total downloads** affected across all MCP SDKs. Anthropic maintains the STDIO execution model is "a secure default" and that sanitization responsibility belongs with developers. **SecurityWeek**: "'By Design' Flaw in MCP Could Enable Widespread AI Supply Chain Attacks." **New: Vulnerable MCP Project** (vulnerablemcp.info) — a comprehensive security database for MCP vulnerabilities has been launched, tracking CVEs and attack patterns. **CVE-2026-39313** added to the vulnerability list. The vulnerability is **cross-SDK**: embedded in Anthropic's official MCP SDKs across Python, TypeScript, Java, and Rust. Ox Security criticized the approach of shifting security responsibility to developers rather than securing the protocol infrastructure.
- **Significance**: The scale upgrade from "200K servers" to "150M downloads" reflects total SDK download exposure, not just active instances. This vulnerability is now receiving enterprise-grade security coverage (Palo Alto Unit 42, SecurityWeek, Infosecurity Magazine) — it's being treated as a systemic AI supply chain risk, not just a protocol bug. For our pipeline: (1) Same assessment as morning — prefer Streamable HTTP over STDIO for Phase 5/7. (2) The emergence of vulnerablemcp.info as a dedicated MCP security database is useful for monitoring. (3) The cross-SDK nature means any MCP SDK version we use is affected by design.
- **Source**: https://www.infosecurity-magazine.com/news/systemic-flaw-mcp-expose-150/, https://www.securityweek.com/by-design-flaw-in-mcp-could-enable-widespread-ai-supply-chain-attacks/, https://vulnerablemcp.info/

### 2026-04-19 (morning) — MCP STDIO Transport Security: Ox Research Team Reports 200K Servers at Risk; Anthropic Declines Responsibility
- **What**: The Register (Apr 16) reported that security researchers from Ox Security identified a systemic vulnerability in MCP's STDIO transport mechanism that allows arbitrary OS command execution via subprocess spawning. Four vulnerability classes identified: (1) unauthenticated command injection, (2) authenticated command injection with hardening bypass, (3) zero-click prompt injection in IDEs, (4) supply chain attacks via poisoned MCP marketplaces. **10 high- and critical-severity CVEs** issued so far, including CVE-2025-65720 (GPT Researcher), CVE-2026-30625 (Upsonic), GHSA-c9gw-hvqq-f33r (Flowise), CVE-2026-30615 (Windsurf). Approximately **200,000 servers** potentially affected. Ox researchers say they "repeatedly" asked Anthropic to patch the root issue; Anthropic responded that the behavior is "expected" protocol function and released updated security guidance recommending caution with STDIO adapters but made no architectural changes. Anthropic did not respond to The Register's inquiries.
- **Significance**: This is a **protocol-level systemic vulnerability**, distinct from the tool-poisoning attacks (OWASP MCP Top 10) and individual server CVEs we've previously tracked. The STDIO transport is the default for local MCP servers — the mechanism Claude Code uses to communicate with local tools. Anthropic's "expected behavior" stance means the vulnerability will persist; security must be implemented at the server/client level, not the protocol. For our pipeline: (1) Our `mcp-scan` integration (already in place) should detect poisoned STDIO servers, but cannot protect against the fundamental subprocess execution model. (2) Phase 5/7 MCP deployments should prefer Streamable HTTP transport over STDIO where possible. (3) The MCP security posture is degrading — 200K exposed servers + "expected behavior" response from the protocol maintainer is a liability signal.
- **Source**: https://www.theregister.com/2026/04/16/anthropic_mcp_design_flaw/

### 2026-04-18 (evening) -- MCP: No Change Since Morning Sweep; 10K+ Servers Stable; Triggers & Events WG Active
- **What**: No new MCP developments since morning sweep. Ecosystem at 10K+ servers. Triggers & Events WG (Clare Liguori/AWS) designing proactive server-to-client notifications. 2026 roadmap priorities unchanged: stateless Streamable HTTP, Server Cards, Tasks primitive, Contributor Ladder, Enterprise readiness. No new spec releases expected before the Tasks primitive finalization.
- **Significance**: MCP in healthy stabilization phase. No action items.
