...
Read all `*.md` files from `~/.claude/@lib/agents/` using Glob. Parse the YAML
...
1. Read the full role definition from `~/.claude/@lib/agents/<role>.md`
...
Consider creating one via meta-agent-factory." and stop.

### Step 4: Role Activation

1. Output: `**Activating role: <role-name>**`
2. Read the full role definition from `~/.claude/@lib/agents/<role>.md`
...
- **Read-only access** to `~/.claude/@lib/agents/` — never create, modify, or delete role files
